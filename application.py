...
Данный пример работает с изображениями в формате BMP. Тест состоит из четырех фаз:
        * инициализация сети
        * обучение сети
        * распознавание изображений
        * анализ эффективности работы

Сначала создается сеть с определенным количеством слоев и нейронов в каждом слое.
Все параметры сети указаны в дескрипторе: resources -> topologies -> bp_test_topology.json
Имя IMAGES_BMP[55-60-10] означает, что в сети три слоя по 55, 60 и 10 нейронов в каждом.

Процесс обучения сети...

Рассмотрим дескриптор подробнее:

{
  "network": {
    "name": "IMAGES_BMP[55-60-10]",
    "auto_rate": true,
    "rate": 0.07,
    "input_size": 10000
  },
  "payload": {
      "template_folder": "group_images_bp",
      "recognize_folder": "number_for_recognize"
  }
}

(Все каталоги и пути относительно папки resources)

В папке с шаблонами template_folder созданы подкаталоги, каждый каталог - отдельная группа образов,
...
Полученный список групп образов сортируется по алфавиту и в ожидаемом отклике сети для i-ой группы i-ый
...
В фазе распознавания сканируется каталог recognize_folder и преобразованные в списки изображения подаются на
...
Конкретно в данном примере сеть уже обучена, поэтому при старте программы запрос на обучение сети можно просто
отклонить, либо согласиться - это не имеет значения, так как времени на этот тратиться не будет.
"""
import os
import re
import time
...
store = StoreService()
if input("Clean Store? [N/y]: ").lower() == 'y':
...
group_identify = {}
group_table = EasyTables(["Group name", "Index of out network"])
logger.debug("Generate mapping...")
for i in range(len(group_list)):
    group_identify[group_list[i]] = i
    group_table.add_row([group_list[i], i])
...
if learn:
    sleep()
    print("##########################################################################")
    print("########## Learn PHASE                    Start in 5 second... ###########")
    print("##########################################################################")
    sleep()
...
print("##########################################################################")
print("########## Recognition PHASE              Start in 5 second... ###########")
print("##########################################################################")
sleep()
...
sleep()
print("##########################################################################")
print("########## Analyze PHASE                  Start in 5 second... ###########")
print("##########################################################################")
sleep()
